...
A test passes when its output contains no `FAIL` line. CI `engine-parity` gates (33 tests):
...
`np_interp` (non-monotonic DIR axis), `gamut_out_aces` + `gamut_in_xy` (opt-in gamut
